-- id | app_name | category | rating | reviews | size | min_installs | price
...
SELECT ID, app_name
FROM  analytics
WHERE last_updated IN ('2018-08-01');
...
SELECT app_name, COUNT(reviews)
FROM analytics
WHERE rating < 3
GROUP BY app_name
HAVING COUNT(reviews) > 10000;


-- 10. Find the top 10 most-reviewed apps that cost between 10 cents and a dollar.
SELECT app_name, price, reviews
FROM analytics
WHERE price > 00.10 AND price < 1.00
ORDER BY reviews DESC
LIMIT 10;
...
SELECT COUNT(reviews) AS 'Total Reviews'
FROM analytics;
